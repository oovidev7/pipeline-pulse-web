...
const ICON_SIZES = { xs: 12, sm: 24, md: 32, lg: 40, xl: 48 }
const TEXT_SIZES = { xs: 12, sm: 18, md: 20, lg: 24, xl: 30 }

function IconMark({ px, color }) {
  return (
    <svg
      viewBox="0 0 123 90"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{ width: px, height: px * (90 / 123), display: 'block' }}
      aria-hidden="true"
    >
      <path
        fillRule="evenodd"
 ...
 */
export function Logo({ variant = 'full', size = 'md', theme = 'light', className, style }) {
  ...
  if (variant === 'icon') {
    return (
      <span className={className} style={style}>
        <IconMark px={iconPx} color={color} />
      </span>
    )
  }
  if (variant === 'wordmark') {
